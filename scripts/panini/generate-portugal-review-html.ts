...
const processedRoot = path.join(root, "processed", "panini", "portugal");
const summaryPath = path.join(processedRoot, "processed-summary.json");
const reviewStatusPath = path.join(root, "data", "panini-portugal-review-status.json");
const sourcesPath = path.join(root, "data", "panini-portugal-sources.json");
...
function ensureReviewStatus(summary: ProcessedSummary) {
...
function main() {
  if (!existsSync(summaryPath)) {
    throw new Error("processed-summary.json not found. Run process-panini-portugal first.");
  }

  mkdirSync(processedRoot, { recursive: true });

  const summary = JSON.parse(readFileSync(summaryPath, "utf8")) as ProcessedSummary;
  const statusRecords = ensureReviewStatus(summary);
...
<head>
  ...
  <title>Panini Portugal Review</title>
  <style>
