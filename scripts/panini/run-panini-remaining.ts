...
function getRosterInfo(country: string) {
  const squadFile = JSON.parse(readFileSync(path.join(root, "data", "fifa-2026-squads.json"), "utf8")) as {
    countries: Array<{ country: string; players: SquadPlayer[] }>;
  };
  const coreCards = JSON.parse(readFileSync(path.join(root, "data", "core-cards.json"), "utf8")) as Record<string, string[]>;
