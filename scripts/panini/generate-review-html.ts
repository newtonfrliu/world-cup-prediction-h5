...
const processedRoot = path.join(root, "processed", "panini", "argentina");
const summaryPath = path.join(processedRoot, "processed-summary.json");
const reviewStatusPath = path.join(root, "data", "panini-argentina-review-status.json");
...
function main() {
  if (!existsSync(summaryPath)) {
    throw new Error("processed-summary.json not found. Run process-panini-argentina first.");
  }

  mkdirSync(processedRoot, { recursive: true });

  const summary = JSON.parse(readFileSync(summaryPath, "utf8")) as ProcessedSummary;
  const statusRecords = existsSync(reviewStatusPath)
...
<head>
  ...
  <title>Panini Argentina Review</title>
  <style>
    body {
      margin: 0;
      background: #f6f1e7;
    ...
    .filters button {
      flex: 0 0 auto;
      border: 1px solid rgba(7,27,58,.16);
      border-radius: 999px;
      background: #fff;
      color: #071b3a;
      padding: 10px 14px;
    ...
    .card.status-pending {
      border-color: #f6c84c;
      box-shadow: 0 12px 34px rgba(246,200,76,.2);
    }
    .card.status-rejected {
      border-color: #e63535;
      box-shadow: 0 12px 34px rgba(230,53,53,.18);
    }
    ...
    .status-badge.status-usable_low_quality {
      background: #e0f2fe;
      color: #075985;
    }
    .flags {
